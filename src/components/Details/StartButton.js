...
const isRecipeInProgress = !!inProgressRecipes[inProgressKey]
  && inProgressRecipes[inProgressKey].hasOwnProperty(itemId);

const setRecipeToInProgress = (recipe) => {
  const recipeObj = recipe[drinksOrMeals][0];
  const ingredients = (counter = 0) => Object.entries(recipeObj).reduce(
    (acc = [], [key, value]) => {
  ...
  return typeRequsition === 'comidas'
    ? saveInProgressRecipes('meals', itemId, ingredients(recipe))
    : saveInProgressRecipes('cocktails', itemId, ingredients(recipe));
};
